    ...
    <form action="" method="POST" enctype="multipart/form-data">
        <?php if ($buttons) : ?>
            <?php foreach ($buttons as $key => $button) : ?>
                <?php if ($button['ajax']) : ?>
                    <a class="btn btn-primary" href="javascript:void(0)" onclick="<?= $this->getButtonUrl($button['method']) ?>"><?= $button['label'] ?></a>
                <?php else : ?>
                    <a href="<?= $this->getButtonUrl()($button['method']) ?>"><?= $button['label'] ?></a>
                <?php endif; ?>
            <?php endforeach; ?>
        <?php endif; ?>
